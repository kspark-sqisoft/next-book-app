...
리뷰 당일 P0 **1~7번을 모두 수정**했다. 각 항목의 원문 진단은 아래 P0 절에 그대로 두고,
무엇을 어떻게 고쳤는지만 여기 요약한다.

| 항목                  | 상태 | 요점                                                                                                                       |
| --------------------- | ---- | -------------------------------------------------------------------------------------------------------------------------- |
| 1. JWT 시크릿         | 해결 | 코드 폴백 제거(프로덕션 기동 실패) · compose 기본값 제거(`:?`) · 시크릿 재발급 · 리프레시 토큰 전량 폐기                   |
...
**1번 처리 방식.** 서명 키에 기본값을 두지 않는다 — 폴백이 있으면 환경변수를 빠뜨린 배포가
공개된 키로 조용히 기동한다. `requiredSecret()`이 프로덕션에서는 미설정·32자 미만이면
throw 하고(기동 실패), 개발·테스트에서는 값이 있으면 그대로 쓰고 없으면 프로세스마다 임의 키를
만든다(재시작 시 세션 끊김을 감수하더라도 저장소에 상수를 박지 않는다).
compose도 `${JWT_ACCESS_SECRET:?...}`로 바꿔 미설정이면 컨테이너가 뜨기 전에 실패한다.
`.env.example`은 플레이스홀더 대신 `openssl rand -base64 48` 생성 안내로 교체했다.
키를 새로 발급했으므로 기존 `refresh_token` 행은 전량 삭제했다 — 옛 키로 서명된 토큰은
어차피 검증되지 않지만 남겨 둘 이유가 없다.

검증: `format:check`·`lint`(0 오류)·`typecheck`(0)·`test:unit`(23파일 107케이스)·`build` 모두 통과.
시크릿 없이 프로덕션으로 `@/server/env`를 로드하면 실제로 throw 하고, 시크릿 없이
`docker compose config`를 돌리면 보간 단계에서 실패하는 것을 확인했다.
