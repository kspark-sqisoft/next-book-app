...
- [~] 6. 대형 컴포넌트 분해 + 에디터 스토어 — ①·③ 완료, ② 진행 중
  ...
  - [ ] ② `BookDetailOwnerView` 계속 분해(현재 파일 2,463줄)
...
`BookDetailOwnerView` 분해가 남았다(파일 2,463줄). 두 화면 사이의 동일 블록은 약 500줄
남았는데, 큰 것이 import 목록(40줄)과 닫는 괄호·props 꼬리라 실질적인 중복은 대부분
걷어냈다. 남은 것은 **한 화면 안의** 분해다.

### 지표 — 이 문서 이후의 변화

| 파일                 | 6번 착수 전 | 현재                       |
| -------------------- | ----------- | -------------------------- |
| `BookDetailPage.tsx` | 3,662       | **2,463**                  |
| `BookEditorPage.tsx` | 1,500       | **1,172**                  |
| 두 화면의 동일 블록  | 609줄       | ~500줄(대부분 import·괄호) |
