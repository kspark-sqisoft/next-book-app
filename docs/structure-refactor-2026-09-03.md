...
- [~] 6. 대형 컴포넌트 분해 + 에디터 스토어 — ①·③ 완료, ② 진행 중
  ...
  - [ ] ② `BookSlideCanvas` 호출부 props 35개 그룹화
  - [ ] ② `BookDetailOwnerView` 3,000줄 분해
...
`BookSlideCanvas` 호출부 props 35개 그룹화와 `BookDetailOwnerView` 3,000줄 분해가 남았다.
③ 을 React Compiler 로 해결해 그룹화가 메모이제이션 목적을 잃었으므로, 남은 근거는 읽기
좋은 코드뿐이다 — 우선순위는 낮다.
