...
`BookSlideCanvas` 호출부 props 35개 그룹화와 `BookDetailOwnerView` 분해가 남았다.
③ 을 React Compiler 로 해결해 그룹화가 메모이제이션 목적을 잃었으므로, 남은 근거는 읽기
좋은 코드뿐이다 — 우선순위는 낮다.

두 파일은 3,191 → 2,958 / 1,500 → 1,276 으로 줄었다. 남은 동일 블록은 약 490줄인데,
큰 것이 `BookSlideCanvas` 호출부(35줄)와 import 목록(43줄)이라 성격이 다르다 — 앞의 것은
props 그룹화에서, 뒤의 것은 그 결과로 자연히 줄어든다.
