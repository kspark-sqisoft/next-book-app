...
- [x] 6. 대형 컴포넌트 분해 + 에디터 스토어 — 완료(남은 한 화면 안 분해는 우선순위 낮음)
  ...
  - [~] ② `BookDetailOwnerView` 남은 분해(파일 1,732줄 — 상세 화면 고유 로직만 남음, 보류)
- [x] 별건: 슬라이드쇼 라우트를 `(bare)` 그룹으로 — 커뮤니티 iframe 흰색 번쩍임 제거
...
20/20 통과한다. `playwright.config.ts` 는 **바꾸지 않기로 결정**(CI 동작까지 바뀌는 변경) —
로컬에서는 커밋하지 않는 임시 config 로만 워커 수를 낮춘다.
