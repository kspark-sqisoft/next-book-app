...
/** 텍스트 위젯 애니메이션 화이트리스트 — 클라이언트와 **같은 정의**를 쓴다(손 동기화 없음) */
const BOOK_TEXT_ANIMATIONS: ReadonlySet<string> = new Set(
...
/**
 * 캔버스 위젯 스키마 — 정의는 클라이언트 쪽 `BookCanvasElement` 하나뿐이다.
 *
 * 이전에는 같은 스키마가 두 벌이었고 서버 쪽이 부분 복사본이라, 위젯 속성이 늘어도
 * 서버 타입은 모르는 채로 남았다(elements 는 JSON 블롭이라 캐스팅만 하므로 컴파일러도
